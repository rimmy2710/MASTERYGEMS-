// backend/src/routes/roomGameV2.ts
...
const roomGameV2Routes: FastifyPluginAsync = async (fastify) => {
  fastify.get("/v2/room-game/health", async () => ({
    ok: true,
    data: { status: "ok", module: "room-game-v2" },
  }));

  // Create (or reuse) active game for a room
  fastify.post("/v2/rooms/:id/game", async (request, reply) => {
    ...
    if (activeId) return reply.send({ ok: true, data: { gameId: activeId, reused: true } });
    ...
    return reply.status(201).send({ ok: true, data: { gameId: res.data!.id, reused: false } });
  });

  // Join room + join active game (or create one)
  fastify.post("/v2/rooms/:id/game/join", async (request, reply) => {
    const roomId = (request.params as { id: string }).id;
  ...
  fastify.post("/v2/rooms/:id/game/ready", async (request, reply) => {
    const roomId = (request.params as { id: string }).id;
    ...
    const res = gameEngine.applyAction(activeId, { type: "PLAYER_READY", playerId: parsed.data.playerId });
  ...
  fastify.post("/v2/rooms/:id/game/unready", async (request, reply) => {
    const roomId = (request.params as { id: string }).id;
    const parsed = readySchema.safeParse(request.body);
    if (!parsed.success) return reply.status(400).send({ ok: false, error: "Invalid payload" });
    ...
    const res = gameEngine.applyAction(activeId, { type: "PLAYER_UNREADY", playerId: parsed.data.playerId });
  ...
  fastify.post("/v2/rooms/:id/game/start", async (request, reply) => {
    const roomId = (request.params as { id: string }).id;
    ...
    const started = gameEngine.applyAction(activeId, { type: "START_GAME", playerId: parsed.data.playerId });
  ...
  fastify.post("/v2/rooms/:id/game/finish", async (request, reply) => {
    const roomId = (request.params as { id: string }).id;
    const parsed = startRoomGameSchema.safeParse(request.body ?? {});
    if (!parsed.success) return reply.status(400).send({ ok: false, error: "Invalid payload" });
    ...
    const finished = gameEngine.applyAction(activeId, { type: "FINISH_GAME", playerId: parsed.data.playerId });
...
export default roomGameV2Routes;
